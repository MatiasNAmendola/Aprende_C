...
int contarNodos(NODO *p)
{
	...
	/*
	Si el puntero que pasamos como parametro
	posee una direccion nula (NULL) devolvera
	cantidad cero.
	*/
	if (p == NULL)
	{
		...
		/* 
		Si el puntero que pasamos como parametro tiene
		un nodo, pero la direccion a la que apunta el
		siguiente o su sucesor es nula (NULL) devolvera
		cantidad 1 porque la lista solo tiene 1 nodo.
		*/		
		if (p->sig == NULL)
        {
			return 1;
